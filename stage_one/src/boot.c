...
#define p_to_str_start(...) #__VA_ARGS__
#define p_to_str(...) p_to_str_start(__VA_ARGS__) 

#define concat_inner(a, b) a ## b
#define concat(a, b) concat_inner(a, b) 

#define v_asm(...) \
  __asm__ __volatile (concat(R, \
        p_to_str( left_brace __VA_ARGS__ right_brace \
          ) \
        ) \
      )  
...
/*
   ...
   */
//#define v_asm(x) __asm__ __volatile__ (R( #x )) 
